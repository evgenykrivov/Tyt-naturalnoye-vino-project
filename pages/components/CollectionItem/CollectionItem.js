...
const CollectionItem = (props) => {
  ...
  return (
    <div className={S.container}>
      <Image className={ S.wineImage } src={ wineImage } alt={ props.name }/>
      <div className={ S.info }>
        { props.item && (
          <>
            <p className={ S.about_bold }>{ props.item.name }</p>
            <p className={ S.about }>Аромат: { props.item.smell.join(", ") }</p>
            <p
              className={ S.about }>Вино { props.item.classification.toLowerCase() } { props.item.species }, { props.item.volume }</p>
